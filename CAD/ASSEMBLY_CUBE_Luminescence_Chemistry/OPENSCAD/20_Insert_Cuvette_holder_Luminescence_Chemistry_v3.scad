...
l_heatspreader = 15+tol;
w_heatspreader = 15+tol;
t_heatspreader = 1+tol;
...
module insert() {
    translate([-l_cuv/2,-w_cuv/2,z_cuv+h/2]) {%cube([l_cuv, w_cuv, h_cuv]); }

    
    translate([l_holder/2-0.01,0,b/2-h/2+zo_lum]) {
        rotate(a=[0,90,0]) {
            difference() {
                union(){
    ...
    // Cuvette holding spring 
    difference(){
        translate([-l_cuv/2,w_cuv/2,h_cuv_spring+h/2])rotate([90,0,-45])translate([-d_cuv_spring-r_cuv_spring,0,-w_cuv_spring/2])cylinder(h=w_cuv_spring,r=r_cuv_spring);
        translate([-l_cuv/2,w_cuv/2,h_cuv_spring+h/2+ndh_cuv_spring])rotate([90,0,-45])translate([-d_cuv_spring-ndd_cuv_spring-r_cuv_spring,0,-w_cuv_spring/2-0.01])cylinder(h=w_cuv_spring+0.02,r=r_cuv_spring+ndr_cuv_spring);
        translate([-2*(r_cuv_spring+ndr_cuv_spring),0,-2*(r_cuv_spring+ndr_cuv_spring)]) cube(2*(r_cuv_spring+ndr_cuv_spring));
        translate([-2*(r_cuv_spring+ndr_cuv_spring),0,ht_cuv_spring+h/2]) cube(2*(r_cuv_spring+ndr_cuv_spring));
    ...
    difference() {
        ...
        // space for cuvette
        translate([-dist_cuv-l_cuv/2,-w_cuv/2,h/2]) {
            cube([l_cuv+dist_cuv, w_cuv+dist_cuv, h_cuv]);
        }

       translate([-l_holder,-2*l_pin,h/2]) cube([l_holder, w_holder, h_holder+0.1]);
       ...
       // heat spreader
      translate([l_cuv/2,0.001-t_heatspreader-w_cuv/2,h/2+z_cuv]) {
          cube([t_heatspreader, w_heatspreader, h_cuv]);
          translate([t_heatspreader-l_heatspreader,0,0]) { 
              cube([l_heatspreader, t_heatspreader, h_cuv]);
          }
      } 
        
        // Luminometer openings
        translate([a/2-l_lum-0.01,0,b/2-h/2+zo_lum]) {  
            rotate(a=[0,90,0]) {
                translate([-d,-w_gap_lum/2,0]) cube([d,w_gap_lum,l_lum+0.02]);
